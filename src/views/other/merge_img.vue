	...
	methods: {
		...
		create_img() {
			...
			if(coupon_price && discount_price){
				price_text='券后价<b>￥'+discount_price+'</b>　<s>原价￥'+price+'</s>';
			}else{
			...
			if(coupon_price && coupon_end_time){
				coupon_text=coupon_price+'元优惠券<br><s>截止时间：'+coupon_end_time+'</s><br><s>长按识别右侧二维码领券</s>';
			}else{
				coupon_text=fomatFloat(price/reserve_price*10, 0)+'折<br><br><s>长按识别右侧二维码购买</s>';
			}
			mc.text(coupon_text,{
				width:'390px',
				align:'left',
